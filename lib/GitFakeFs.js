var Path = require('path'),
    ...
    OutsideWorkingCopyError = createError({
        name: 'OUTSIDEWORKINGCOPY'
    });
                                        ...
                                        if (fsEntry.type === 'directory') {
                                            ...
                                            fs.readlink(fsEntry.fsPath, passError(cb, function (linkString) {
                                                ...
                                                if (/^\.\.\//.test(workDirRelativePath)) {
                                                    return cb(new OutsideWorkingCopyError({
                                                        message: 'fallBackToWorkingCopy: Symbolic links that point outside the working copy are not supported yet: ' + fsEntry.fsPath + ' => ' + linkString + ' (' + workDirRelativePath + ')',
                                                        relativeTargetPath: workDirRelativePath
                                                    }));
                                                }
                        ...
                        if (entry.name === entryName) {
                            entry.getTarget(passError(cb, function (target) {
                                if (entry.type === 'symbolicLink' && dereferenceSymbolicLinks) {
                                    var targetStr = target.toString('utf-8');
                                    // Check if the symbolic link points out of the repo:
                                    var relativeTargetPath = Path.join(fragments.join(Path.sep), targetStr);
                                    if (relativeTargetPath == '..' || relativeTargetPath.indexOf('..' + Path.sep) === 0) {
                                        return cb(new OutsideWorkingCopyError({
                                            relativeTargetPath: relativeTargetPath
                                        }));
                                    }
                                    var symbolicLinkTargetPath = normalizePath(Path.resolve(Path.sep + fragments.join(Path.sep), targetStr));
