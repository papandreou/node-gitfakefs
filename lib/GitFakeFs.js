...
var GitFakeFs = module.exports = function GitFakeFs(repositoryOrPath, config) {
    ...
    that.getTreeOrBlobWhileFollowingSymbolicLinks = function (path, maxSymbolicLinks, cb) {
        if (typeof maxSymbolicLinks !== 'number') {
            cb = maxSymbolicLinks;
            maxSymbolicLinks = 40;
        }
        (function lookUpPath(path) {
            that.getTreeOrBlob(path, passError(cb, function (treeOrBlob, entry) {
                if (treeOrBlob instanceof nodeGit.Tree || treeOrBlob instanceof FakeTree) {
                    cb(null, treeOrBlob, entry);
                } else {
                    ...
                    if (isSymbolicLink) {
                        if (maxSymbolicLinks > 0) {
                            var symbolicLinkTarget = Path.resolve(path.replace(/\/[^\/]+$/, '/'), treeOrBlob.content().toString('utf-8'));
                            maxSymbolicLinks -= 1;
                            lookUpPath(symbolicLinkTarget);
                        } else {
    ...
    that.readdir = function (dirName, cb) {
        that.getTreeOrBlobWhileFollowingSymbolicLinks(dirName, passError(cb, function (tree) {
    ...
    // TODO: Report the originally stat'ed fileOrDirName in the error message when the target of a symbolic link cannot be found
    that.stat = function (fileOrDirName, maxSymbolicLinks, cb) {
        if (typeof maxSymbolicLinks !== 'number') {
            cb = maxSymbolicLinks;
            maxSymbolicLinks = 40;
        }
            ...
            if (stats.isSymbolicLink()) {
                if (maxSymbolicLinks > 0) {
                    that.readFile(fileOrDirName, 'utf-8', passError(cb, function (contents) {
                        var symbolicLinkTarget = Path.resolve(fileOrDirName.replace(/\/[^\/]+$/, '/'), contents);
                        that.stat(symbolicLinkTarget, maxSymbolicLinks - 1, cb);
                    }));
                } else {
                    cb(new fakeFsErrors.Eloop(that.toString() + ' Error: ELOOP, too many symbolic links encountered' + " '" + fileOrDirName + "'"));
                }
