                ...
                for (var i = 0 ; i < parentEntries.length ; i += 1) {
                    ...
                    if (parentEntry.name() === lastFragment) {
                        if (parentEntry.isBlob() || !parentEntry.isTree()) {
                            ...
                            cb(new Error('Internal error, object is neither blob nor tree: ' + parentEntry));
                        }
                        return;
                    }
                }
        ...
        that.lstat(fileOrDirName, passError(cb, function (stats) {
            if (stats.isSymbolicLink()) {
                if (maxSymlinks > 0) {
                    ...
                    cb(new fakeFsErrors.Eloop('Error: ELOOP, too many symbolic links encountered' + " '" + fileOrDirName + "'"));
                }
            } else {
                cb(null, stats);
            }
