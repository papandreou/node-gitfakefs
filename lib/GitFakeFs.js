...
var GitFakeFs = module.exports = function GitFakeFs(repositoryOrPath, config) {
    ...
    that.getTreeOrBlob = memoizeAsync(function (path, dereferenceSymbolicLink, cb) {
        ...
        if (config.index) {
            ...
            that.getTreeOrBlob('/' + fragments.join('/'), true, passError(cb, function (parentTree, parentParentEntry, symbolicLinkNestingLevel, seenSymbolicLinks) {
                symbolicLinkNestingLevel = symbolicLinkNestingLevel || 0;
                        ...
                        if (parentEntry.isBlob() || !parentEntry.isTree()) {
                            if (parentEntry.filemode() === 0xe000) { // Ignore submodules
                                ...
                                parentEntry.getBlob(passError(cb, function (blob) {
                                    if (parentEntry.filemode() === 0120000 && dereferenceSymbolicLink) {
                                        var symbolicLinkTarget = normalizePath(Path.resolve('/' + fragments.join('/'), blob.content().toString('utf-8')));
                                        if (symbolicLinkTarget === path || (seenSymbolicLinks && seenSymbolicLinks.indexOf(symbolicLinkTarget) !== -1) || symbolicLinkNestingLevel > 40) {
                                            return cb(new fakeFsErrors.Eloop(that.toString() + ' Error: ELOOP, too many symbolic links encountered' + " '" + path + "'"));
                                        }
                                        that.getTreeOrBlob(symbolicLinkTarget, true, passError(cb, function (treeOrBlob, symbolicLinkEntry, symbolicLinkTargetNestingLevel, symbolicLinkSeenSymbolicLinks) {
                                            cb(null, treeOrBlob, symbolicLinkEntry, symbolicLinkNestingLevel + 1, (seenSymbolicLinks || []).concat(symbolicLinkTarget));
                                        }));
                                    } else {
                                        cb(null, blob, parentEntry, symbolicLinkNestingLevel, seenSymbolicLinks);
                                    }
                                }));
                            }
                        } else if (parentEntry.isTree()) {
                            parentEntry.getTree(passError(cb, function (tree) {
                                cb(null, tree, parentEntry, symbolicLinkNestingLevel, seenSymbolicLinks);
                            }));
                        } else {
