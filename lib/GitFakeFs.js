...
var GitFakeFs = module.exports = function GitFakeFs(repositoryOrPath, config) {
            ...
            repo.openIndex(passError(cb, function (index) {
                ...
                index.entries().forEach(function (indexEntry) {
                    if (indexEntry.mode() !== 0xe000) { // Ignore submodules
                        ...
                        if (!(dirName in treeByDirName)) {
                            var fakeTree = treeByDirName[dirName] = new FakeTree([indexEntry]),
                            ...
                            treeByDirName[dirName].entries().push(indexEntry);
                        }
                    }
                });
    ...
    that.readFile = function (fileName, encoding, cb) {
        ...
        that.getTreeOrBlob(fileName, true, function (err, blob) {
            if (err) {
                if (config.fallBackToWorkingCopy) {
                    that.getRepo(passError(cb, function (repo) {
                        ...
                        if (!workdir) {
                            ...
                            fs.readFile(Path.resolve(workdir, fileName.substr(1)), encoding, cb);
                        }
                    }));
                } else {
                    cb(err);
    ...
    that.readdir = function (dirName, cb) {
        that.getTreeOrBlob(dirName, true, function (getTreeOrBlobErr, tree) {
            var names = [];
            if (!getTreeOrBlobErr) {
                if (!isTree(tree)) {
            ...
            if (config.fallBackToWorkingCopy) {
                that.getRepo(passError(cb, function (repo) {
                    ...
                    fs.readdir(Path.resolve(workdir, dirName.substr(1)), function (fsReaddirErr, workingCopyNames) {
    ...
    ['stat', 'lstat'].forEach(function (methodName) {
        that[methodName] = function (fileOrDirName, cb) {
            that.getTreeOrBlob(fileOrDirName, methodName === 'stat', function (err, treeOrBlob, treeEntry) {
                if (err) {
                    if (config.fallBackToWorkingCopy) {
                        that.getRepo(passError(cb, function (repo) {
                            var workdir = repo.workdir();
                            ...
                            fs[methodName](Path.resolve(workdir, fileOrDirName.substr(1)), cb);
                        }));
                        return;
                    } else {
                        return cb(err);
    ...
    that.realpath = function (fileOrDirName, cb) {
        that.getTreeOrBlob(fileOrDirName, true, function (err, treeOrBlob, treeEntry) {
            if (err) {
                if (config.fallBackToWorkingCopy) {
                    that.getRepo(passError(cb, function (repo) {
                        var workdir = repo.workdir();
                        ...
                        fs.realpath(Path.resolve(workdir, fileOrDirName.substr(1)), passError(cb, function (realpathInWorkdir) {
