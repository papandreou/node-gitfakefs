var GitFakeFs = require('./GitFakeFs'),
    ...
    virtualContentsDirectories = ['HEAD', 'branches', 'tags', 'commits', 'index'];
...
var GitWrappedFs = module.exports = function GitWrappedFs(fs) {
    ...
    function acquireLock(callbackToWrap, cb) {
        cb.callbackToWrap = callbackToWrap;
        waitingQueue.push(cb);
        proceed();
    }

    function proceed() {
        while (numInFlight < maxConcurrencyLevel && waitingQueue.length > 0) {
            numInFlight += 1;
            var nextCb = waitingQueue.shift(),
                callbackCalled = false;
            nextCb(function wrappedCb() {
                nextCb.callbackToWrap.apply(this, arguments);
                numInFlight -= 1;
                proceed();
            });
        }
                ...
                if (matchGitRepoInPath) {
                    ...
                    if (!matchGitRepoInPath[2]) {
                        if (fsMethodName === 'readdir') {
                            // Intercept the result and add add the 'gitFakeFs' dir
                    ...
                    getRepo(pathToRepository, passError(cb, function (repo) {
                        if (additionalFragments.length === 0) {
                            if (fsMethodName === 'readdir') {
                                ...
                                cb(new Error('GitWrappedFs: ' + fsMethodName + ' not supported on the virtual gitFakeFs directory'));
                            }
                        } else if (additionalFragments.length >= 1) {
                            acquireLock(cb, function (cb) {
                                if (typeof lastArgument === 'function') {
                                    args[args.length - 1] = cb;
                                }

                                var objectType = additionalFragments.shift();
                                if (objectType === 'index' || objectType === 'HEAD') {
                                    // No further levels
                                    return proxyToGitFakeFs({index: objectType === 'index', ref: 'HEAD'});
                                }
